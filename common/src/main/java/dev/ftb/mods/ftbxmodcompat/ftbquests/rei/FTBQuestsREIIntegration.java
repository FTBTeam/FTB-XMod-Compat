...
public class FTBQuestsREIIntegration implements REIClientPlugin {
    private static BasicFilteringRule.@Nullable MarkDirty cratesChanged;
    ...
    public void registerDisplays(DisplayRegistry registry) {
        if (FTBXModCompat.isFTBQuestsLoaded) {
            registry.beginFiller(WrappedQuest.class).fill(QuestDisplay::new);
            registry.beginFiller(WrappedLootCrate.class).fill(LootCrateDisplay::new);
    ...
    public static void showRecipes(FluidStack fluid) {
        dev.architectury.fluid.FluidStack archFluid = dev.architectury.fluid.FluidStack.create(fluid.fluid(), fluid.amount(), fluid.getComponentsPatch());
        show(VanillaEntryTypes.FLUID, archFluid);
    }
